...
class BaseDataset(Dataset):
    ...
    def subsets(self, sub_ids, sub_args_lst=None):
        ...
        indices = [[id for id in ids] for ids in sub_ids]     # for ids in sub_ids is to choose classes
        ...
        return subsets    # it's a list of Subset dataset
...
class PairedDataset(Dataset):
    ...
    def __init__(self, datasets, n_elements, max_iters, same=True,
        ...
        if same:
            if not isinstance(self.n_elements, list):
...
class Subset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        return self.dataset[self.indices[idx]] # return a list of a certain class
